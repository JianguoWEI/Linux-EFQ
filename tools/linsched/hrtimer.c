...
unsigned int total_energy_consumption = 0;
...
void linsched_run_sim(int sim_ticks)
{
	...
	while (current_time < KTIME_MAX && total_energy_consumption < 50000
		...
		for_each_cpu(active_cpu, &runnable) {
			...
			printf("\ncurrent time = %llu\n", current_time);
			detailed_info = fopen("detailed_info.txt","a+");
			fprintf(detailed_info,"%d\n", (current_time-100)/NSEC_PER_MSEC);
			fclose(detailed_info);
			...
			if((current_time-100)%200000000 ==0)
				linsched_print_task_stats_periodical();
			/*if((current_time-100)%10000000 == 0)
				printf("\n******* PERIOD of TASK 4 *******\n");*/
		}
	}
}
