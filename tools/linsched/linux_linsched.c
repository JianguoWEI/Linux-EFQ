...
void linsched_print_task_stats(void)
{
	int i;
	long total_time = 0;
	long system_active_time = (current_time-100)/NSEC_PER_MSEC;
	FILE *output;
	output = fopen("output.txt","a+");
	fprintf(output,"%llu\t", (current_time-100)/NSEC_PER_MSEC);
	FILE *totaltime_deadline_response;
	totaltime_deadline_response = fopen("totaltime_deadline_response.txt","a+");

	for (i = 1; i < num_tasks; i++) {
		struct task_struct *task = __linsched_tasks[i];
		struct task_data *td = task_thread_info(task)->td;
		struct sleep_run_task *d = td->data;
		float response_average = d->response_cumulative/(d->n_period-1);
		printf
		    ("Task id = %d (%d), exec_time = %llu, run_delay = %llu, pcount = %lu,\n		 n_period = %d, n_deadline_met = %d, n_deadline_missed = %d,\n   		response_average = %f, response_maximum =%d\n", task_pid_nr(task), i, task_exec_time(task), task->sched_info.run_delay, task->sched_info.pcount, d->n_period-1, d->n_deadline_met, d->n_deadline_missed, response_average, d->response_maximum);
		printf ("		consumed_energy = %d, share_of energy = %f, ave_power = %f\n", task->se.sum_consumed_energy, (float)(task->se.sum_consumed_energy)/total_energy_consumption,(float)(task->se.sum_consumed_energy)/system_active_time);
		fprintf(output,"%d\t", task->se.sum_consumed_energy);
		total_time += task_exec_time(task);
	}

	fprintf(totaltime_deadline_response,"%ld\t", system_active_time);

	for (i = 1; i < num_tasks; i++) {
			struct task_struct *task = __linsched_tasks[i];
			fprintf(output,"%f\t", (float)(task->se.sum_consumed_energy - (task->se.prev_sum_consumed_energy))/(system_active_time%100)); // output the average power during the past 100ms
	}
	...
	for (i = 1; i < num_tasks; i++) {
			...
			struct task_data *td = task_thread_info(task)->td;
			struct sleep_run_task *d = td->data;
			fprintf(output,"%f\t", (float)(task->se.sum_consumed_energy)/total_energy_consumption); // output the average power during the past 100ms
			if(d->task_type==0)
				fprintf(totaltime_deadline_response,"%d\t%d\t", d->n_period-1, d->n_deadline_missed);
			if(d->task_type==1)
				fprintf(totaltime_deadline_response,"%f\t%d\t", d->response_cumulative/(d->n_period-1), d->response_maximum);
	}

	fprintf(output,"\n");
	fprintf(totaltime_deadline_response,"\n");

	printf("Total exec_time = %ld\n", total_time);
	printf("Total energy_consumption = %d\n", total_energy_consumption);
	printf("System total active time = %ld\n", system_active_time);
	printf("Average power = %f\n", (float)(total_energy_consumption)/system_active_time);
}
...
void linsched_print_task_stats_periodical(void)
{
	...
	for (i = 1; i < num_tasks; i++) {
		struct task_struct *task = __linsched_tasks[i];
		fprintf(output,"%f\t", (float)(task->se.sum_consumed_energy - task->se.prev_sum_consumed_energy)/200); // output the average power during the past 100ms
		task->se.prev_sum_consumed_energy = task->se.sum_consumed_energy ;
	}
...
/* JW-GDEM 2013-02-27
 * Create a EFQ general task with the specified callback and
 * nice value of niceval, which determines its weight and energy share
 */
struct task_struct *linsched_create_EFQ_task_general(struct task_data *td, int niceval)
{
	...
	/* Create EFQ task and set its nice value. */
	p = __linsched_tasks[id] = __linsched_create_task(td);
	__linsched_set_task_id(p, id);

	params.sched_priority = 0;
	params.warp_value = 0;
	params.warp_time_limit = 0;
...
/* JW-GDEM 2013-02-27
 * Create a EFQ batch task with the specified callback and
 * nice value of niceval, which determines its weight and energy share
 */
...
/* ***********************************************
 * Create task with parameters of share and weight
 */
...
struct task_struct *linsched_create_EFQ_task_batch_weight(struct task_data *td, float weight)
{
	...
	params.warp_time_limit = 0;
	sched_setscheduler(p, SCHED_EFQ, &params);
	recalculate_set_user_nice(p,0,weight); // JW-GDEM
	assert(current->cgroups->subsys[0]);
	assert(p->cgroups->subsys[0]);
	return p;
}

struct task_struct *linsched_create_EFQ_task_rt_interactive_share(struct task_data *td, float reserved_share, u64 warp_value, int warp_time_limit, int unwarp_time)
{
	struct sched_param params = {};
	struct task_struct *p;
	int id = num_tasks++;
	...
	params.warp_value = warp_value;
	params.warp_time_limit = warp_time_limit;
	params.unwarp_time = unwarp_time;
	sched_setscheduler(p, SCHED_EFQ, &params);
	recalculate_set_user_nice(p,reserved_share,0); // JW-GDEM
	assert(current->cgroups->subsys[0]);
	assert(p->cgroups->subsys[0]);
	return p;
}
...
struct task_struct *linsched_create_EFQ_task_interactive_weight(struct task_data *td, float initial_weight, u64 warp_value, int warp_time_limit, int unwarp_time)
{
	...
	params.unwarp_time = unwarp_time;
	sched_setscheduler(p, SCHED_EFQ, &params);
	recalculate_set_user_nice(p,0,initial_weight); // JW-GDEM
	assert(current->cgroups->subsys[0]);
	assert(p->cgroups->subsys[0]);
	return p;
}
...
{
	u64 reserved_share_fp = reserved_share*fixed_point_scale; // change to fixed point
	u64 initial_weight_fp = initial_weight*fixed_point_scale; // change to fixed point
	//recalculate_set_user_nice_kernel(p,reserved_share,initial_weight);
	recalculate_set_user_nice_kernel(p,reserved_share_fp,initial_weight_fp);
}
...
static void sleep_run_sleep_for(struct sleep_run_data *d, int state, void *data)
{
	struct task_struct *p = d->p;
	struct sched_entity *se = &p->se;
	struct sleep_run_task *ddy = data; // data type that contains dynamic sleep run time
	u64 sleep = ddy->sleep * NSEC_PER_MSEC;

	se->comsumed_energy_IO = 0;
	int i;

	hrtimer_set_expires(&d->timer, ns_to_ktime(sleep));
	hrtimer_start_expires(&d->timer, HRTIMER_MODE_REL);
	printf("Zzzz...sleep for %d ms\n",ddy->sleep);
	...
	// Add energy consumption during I/O sleep time
	for(i=0; i< ddy->sleep; i++){
		se->comsumed_energy_IO += random(ddy->min_IO_power,ddy->max_IO_power);
	}

	printf("Zzzz.... energy consumption during I/O is %d\n", se->comsumed_energy_IO );
}

/* Go to sleep completely */
static void force_to_sleep(struct sleep_run_data *d, int state)
{
	if(d->p) {
		d->p->state = state;
		schedule();
...
static void rt_sleep_run_sleep_for(struct sleep_run_data *d, int state, void *data)
{
	struct task_struct *p = d->p;
	struct sched_entity *se = &p->se;
	struct sleep_run_task *ddy = data; // data type that contains sleep,run and period time
	u64 sleep = ddy->sleep * NSEC_PER_MSEC;
	...
	for(i=0; i< ddy->IO_time; i++){
		se->comsumed_energy_IO += ddy->IO_power;
	}
...
static int sleep_run_run_for(struct sleep_run_data *d,  u64 busy)
{
	struct task_struct *p = d->p;
	struct sched_entity *se = &p->se;

	/*if(p->pid == 5 && se->sum_consumed_energy >= 15000){
			force_to_sleep(d,TASK_DEAD);
	}*/
/*
	if(p->pid == 5 && se->sum_consumed_energy >= 18000){
				force_to_sleep(d,TASK_DEAD);
	}
*/
	...
	hrtimer_set_expires(&d->timer, ns_to_ktime(busy - delta));
	hrtimer_start_expires(&d->timer, HRTIMER_MODE_REL);
	return 0;
}

// for rt and interactive task
static int rt_sleep_run_run_for(struct sleep_run_data *d, void *data)
{
	struct task_struct *p = d->p;
	struct sched_entity *se = &p->se;
	struct sleep_run_task *ddy = data; // data type that contains dynamic sleep run time
	u64 busy = ddy->busy * NSEC_PER_MSEC;
	//u64 period = ddy->period * NSEC_PER_MSEC;
	u64 elapsed_time = current_time - ddy->task_create_time;
	u64 cumulative_period_time = ddy->cumulative_period_time * NSEC_PER_MSEC;
	FILE *response_time;


	//response_time = fopen("response_time.txt","a+");

	if(p->pid == 3 && se->sum_consumed_energy >= 24000)
					force_to_sleep(d,TASK_INTERRUPTIBLE);
	...
	if (delta >= busy) {
		...
		printf("cumulative_period_time is %llu, elapsed_time is %llu\n", cumulative_period_time, elapsed_time);
		printf("**********  Task %d finish the work of Period %d  **********\n", p->pid, ddy->n_period);

		if((cumulative_period_time - ddy->IO_time) >= elapsed_time)
		//if(cumulative_period_time >= elapsed_time)
		{
			ddy->sleep = (cumulative_period_time - elapsed_time)/NSEC_PER_MSEC;
			printf("Task %d goes to sleep %d ms...\n", p->pid, ddy->sleep);
			if(ddy->task_type==0) // real-time task
			{
				ddy->n_deadline_met ++;
				printf("MEET dealine = %d\n", ddy->n_deadline_met);
			}
			
		}
		else
		{
			ddy->sleep =  ddy->IO_time;
			//ddy->sleep = 0;
			printf("Task %d delayed to the next Period ...\n", p->pid);
			if(ddy->task_type==0) // real-time task
			{
				ddy->n_deadline_missed ++;
				printf("MISS dealine = %d\n", ddy->n_deadline_missed);
			}
			else //interactive task, cumulative period time needs to be updated to the elapsed time
				//ddy->cumulative_period_time = elapsed_time/NSEC_PER_MSEC;
				ddy->cumulative_period_time = (elapsed_time/NSEC_PER_MSEC) + ddy->IO_time;
		}

		ddy->n_period ++;

		//if(p->pid == 3 &&  3000>= (current_time-100)/NSEC_PER_MSEC >= 2000)

		if(p->pid == 3 && (current_time-100)/NSEC_PER_MSEC >= 993 && (current_time-100)/NSEC_PER_MSEC < 1993)
			ddy->busy = 9;
		else
			ddy->busy = random(ddy->min_busy,ddy->max_busy);
		
		if(ddy->task_type==1){ // if interactive task, the length of next period needs to be re-generated
			ddy->response = (elapsed_time/NSEC_PER_MSEC) + ddy->IO_time - (cumulative_period_time/NSEC_PER_MSEC - ddy->period);
			//ddy->response = elapsed_time/NSEC_PER_MSEC - (cumulative_period_time/NSEC_PER_MSEC - ddy->period);
			response_time = fopen("response_time.txt","a+");
			fprintf(response_time,"%d\t%f\n", (current_time-100)/NSEC_PER_MSEC, ddy->response);
			fclose(response_time);

			ddy->response_cumulative += ddy->response;
			if(ddy->response_maximum < ddy->response)
				ddy->response_maximum = ddy->response;

			ddy->period = random(ddy->min_period,ddy->max_period);
			printf("**********  Task %d(Interactive) Length of Period %d is %d  ************\n", p->pid, ddy->n_period, ddy->period);
		}
		printf("**********  Task %d Workload of Period %d is %d  ************\n", p->pid, ddy->n_period, ddy->busy);

		ddy->cumulative_period_time += ddy->period;
		return 1;
	}
...
static void sleep_run_handle(struct task_struct *p, void *data)
{
	...
	se->energy_packet_size = random(d->min_energy_packet_size, d->max_energy_packet_size);
	printf("Task %d, inside run_run, energy_packet_size = %d\n", p->pid, se->energy_packet_size );
	total_energy_consumption = se->cfs_rq->total_energy_consumption;

	if(d->sleep_time_fixed){  // in case of non-RT tasks
	...
	else // in case of RT or Interactive task
	{
		if (rt_sleep_run_run_for(&d->sr_data, d)) {
			if(d->sleep) {
					rt_sleep_run_sleep_for(&d->sr_data, TASK_INTERRUPTIBLE,
								d);
					} else {
						rt_sleep_run_run_for(&d->sr_data, d);
					}
			}
	}
...
struct task_data *linsched_create_sleep_run_rt(int period, int busy_average, int busy_range,int energy_packet_size_ave, int energy_packet_size_range)
{
 ...
 printf("**********  Workload of Period %d is %d  ************\n", d->n_period, d->busy);
	return td;
}
...
struct task_data *linsched_create_sleep_run_interactive(int period_average, int period_range, int busy_average, int busy_range, int energy_packet_size_ave, int energy_packet_size_range)
{
	...
	d->n_period = 1; // number of periods the task has been executed
printf("**********  Length of Period %d is %d  ************\n", d->n_period, d->period);
	d->sleep = d->period - d->busy;
	d->cumulative_period_time = d->period;
	d->n_deadline_missed = 0;
	d->n_deadline_met = 0;
...
struct task_data *linsched_create_sleep_run_energy(int sleep, int busy, int energy_packet_size_ave, int energy_packet_size_range, int IO_power_ave,int IO_power_range)
{
	...
	d->min_energy_packet_size = energy_packet_size_ave - energy_packet_size_range;
	d->max_energy_packet_size = energy_packet_size_ave + energy_packet_size_range;
	d->min_IO_power = IO_power_ave - IO_power_range;
	d->max_IO_power = IO_power_ave + IO_power_range;
	d->task_type = 2;
	d->sleep_time_fixed = 1;
	d->sleep = sleep;
	d->busy = busy;
	sleep_run_init(&d->sr_data);
	td->data = d;
...
static void rnd_dist_sleep_run_handle(struct task_struct *p, void *data)
{
	...
	if (sleep_run_run_for(&d->sr_data, d->busy)) {
		...
		if (!d->sleep)
			d->sleep = 1;
		sleep_run_sleep_for(&d->sr_data, TASK_INTERRUPTIBLE, d);
	}
}
...
static void perf_task_handle(struct task_struct *p, void *data)
{
	...
	if (sleep_run_run_for(&d->sr_data, d->busy)) {
		...
		if (d->sleep) {
			...
			sleep_run_sleep_for(&d->sr_data, state, d);
		}
	}
}
